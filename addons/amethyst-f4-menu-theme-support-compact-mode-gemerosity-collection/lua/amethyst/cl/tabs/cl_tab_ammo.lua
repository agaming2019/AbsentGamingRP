...
function PANEL:Init()
	...
	Amethyst.Value = GAMEMODE.AmmoTypes[1]
    ...
    self.DPanel_SectionTop.Paint = function(self, w, h)
		...
		draw.DrawText( string.upper(Amethyst.Language["ammo"] or "AMMO"), "Amethyst.Font.SectionTitle", 23, 5, Color(GetConVar("amethyst_m_tcolor_red"):GetInt(), GetConVar("amethyst_m_tcolor_green"):GetInt(), GetConVar("amethyst_m_tcolor_blue"):GetInt(), GetConVar("amethyst_m_tcolor_alpha"):GetInt()) or Color(255, 255, 255, 255), TEXT_ALIGN_LEFT )
    end
		...
		if doHasCategory then
			...
			for k, v in pairs(GAMEMODE.AmmoTypes) do
				...
				if v.category != nil then
					if v.category == doCatResult.name then table.insert(doCategoryAmmoList, v) end
				end
			end

			local doCategoriesCalcHeight = 0
			for k, v in pairs( doCategoryAmmoList ) do
				doCategoriesCalcHeight = math.ceil( table.Count( doCategoryAmmoList ) / 2 ) * doHeightCalc + 40
			end
			...
			for k, v in pairs( doCategoryAmmoList ) do
	            ...
	            self.DModelPanel_ItemModel:SetPos( v.amethyst_offsetW or 7, v.amethyst_offsetH or 3 )
	            self.DModelPanel_ItemModel:SetSize( 60, 60 )
		        self.DModelPanel_ItemModel._Paint = self.DModelPanel_ItemModel.Paint

				local doHeightCalcTotal = math.ceil( table.Count( doCategoryAmmoList ) / 2 ) * doHeightCalc or 80
    ...
    self.DButton_Item_Action.PerformLayout = function()
        ...
        self.DButton_Item_Action.DoClick = function()
            RunConsoleCommand("darkrp", "buyammo", Amethyst.Value.ammoType)
        end
    end
...
vgui.Register("Amethyst_Tab_Ammo", PANEL, "DPanel")
