...
function PANEL:Init()
    ...
    self.DPanel_SectionTop.Paint = function(self, w, h)
		...
		draw.DrawText( string.upper(Amethyst.Language["entities"] or "ENTITIES"), "Amethyst.Font.SectionTitle", 23, 5, Color(GetConVar("amethyst_m_tcolor_red"):GetInt(), GetConVar("amethyst_m_tcolor_green"):GetInt(), GetConVar("amethyst_m_tcolor_blue"):GetInt(), GetConVar("amethyst_m_tcolor_alpha"):GetInt()) or Color(255, 255, 255, 255), TEXT_ALIGN_LEFT )
    end
	...
	-- All categories available within entities
	for _, doCatResult in pairs( doFetchCategories ) do
		...
		for k, v in pairs( DarkRPEntities ) do
			...
			-- if entities category is the same category as the looped category, set doHasCategory to true
		...
		if doHasCategory then 	-- if the item has doHasCategory

			local doCategoryEntitiesList = {}
			...
			for k, v in pairs(DarkRPEntities) do
				...
				if v.category != nil then
					-- if category is the same as looped category name, insert into categoryentitieslist
					if v.category == doCatResult.name then table.insert(doCategoryEntitiesList, v) end
				end
			end

			local doCategoriesCalcHeight = 0
			for k, v in pairs( doCategoryEntitiesList ) do
				doCategoriesCalcHeight = math.ceil( table.Count( doCategoryEntitiesList ) / 2 ) * doHeightCalc + 40
			end
			...
			for k, v in pairs( doCategoryEntitiesList ) do
				...
				if (v.allowed and table.HasValue(doPopulateJobs, LocalPlayer():Team())) or not v.allowed then
					...
					self.DPanel_Job.Paint = function( self, w, h )
						...
						draw.SimpleText(v.name, "Amethyst.Font.CardItemTitle", 90, h / 2 - 20, DrawBoxTextColor or Color(255, 255, 255, 255))
						draw.SimpleText(GAMEMODE.Config.currency .. ( v.price ), "Amethyst.Font.JobSalary", 90, h / 2 + 4, DrawBoxTextColor or Color(255, 255, 255, 255))

						draw.RoundedBox(0, 0, 2, 75, h - 4, Color(0, 0, 0, 80))
						draw.RoundedBox(4, w - 75, h / 2 - 11, 55, 25, DrawSecondaryColor)

						draw.SimpleText(fetchItemMax .. " Max", "Amethyst.Font.CardItemMax", w - 48, h / 2 + 1, DrawBoxTextColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
					end
		            ...
		            if isstring(doDefaultJobModel) then
		            ...
		            self.DModelPanel_ItemModel:SetFOV(v.amethyst_offsetfov or 60)
		            self.DModelPanel_ItemModel:SetCamPos(Vector(size, size, size))
		            self.DModelPanel_ItemModel:SetLookAt((mn + mx) * 0.5)
		            self.DModelPanel_ItemModel:SetPos( v.amethyst_offsetW or 7, v.amethyst_offsetH or 4 )
		            self.DModelPanel_ItemModel:SetSize( 60, 60 )
			        self.DModelPanel_ItemModel._Paint = self.DModelPanel_ItemModel.Paint

					local doHeightCalcTotal = math.ceil( table.Count( doCategoryEntitiesList ) / 2 ) * doHeightCalc or 70
    ...
    if isstring(Amethyst.Value.model) then
        self.DModelPanel_Item_Model:SetModel(Amethyst.Value.model)
    else
    ...
    self.DModelPanel_Item_Model:SetLookAt((mn + mx) * .0)
    ...
    self.DLabel_Item_Desc.PerformLayout = function()
    	local EntDescText = "No additional info"
    	if Amethyst.Settings.CustomEntity[Amethyst.Value.ent] then
			EntDescText = Amethyst.Settings.CustomEntity[Amethyst.Value.ent].EntSetDesc or ""
		end
    ...
    self.DButton_Item_Action.PerformLayout = function()
        ...
        self.DButton_Item_Action.DoClick = function()
            RunConsoleCommand("darkrp", Amethyst.Value.cmd)
        end
    end
...
vgui.Register("Amethyst_Tab_Entities", PANEL, "DPanel")
