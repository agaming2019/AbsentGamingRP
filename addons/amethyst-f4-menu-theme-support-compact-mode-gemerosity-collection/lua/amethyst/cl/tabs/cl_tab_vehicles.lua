...
function PANEL:Init()
	...
	local doFetchCategories = {category = { name = "Other" } }
	...
	Amethyst.Value = CustomVehicles[1]
    ...
    self.DPanel_SectionTop.Paint = function(self, w, h)
		...
		draw.DrawText( string.upper(Amethyst.Language["vehicles"] or "VEHICLES"), "Amethyst.Font.SectionTitle", 23, 5, Color(GetConVar("amethyst_m_tcolor_red"):GetInt(), GetConVar("amethyst_m_tcolor_green"):GetInt(), GetConVar("amethyst_m_tcolor_blue"):GetInt(), GetConVar("amethyst_m_tcolor_alpha"):GetInt()) or Color(255, 255, 255, 255), TEXT_ALIGN_LEFT )
    end
		...
		if doHasCategory then
			...
			for k, v in pairs(CustomVehicles) do
				...
				if v.category != nil then
					if v.category == doCatResult.name then table.insert(doCategoryVehicleList, v) end
				end
			end

			local doCategoriesCalcHeight = 0
			for k, v in pairs( doCategoryVehicleList ) do
				doCategoriesCalcHeight = math.ceil( table.Count( doCategoryVehicleList ) / 2 ) * doHeightCalc + 40
			end

			PrintTable(doCatResult)
			...
			for k, v in pairs( doCategoryVehicleList ) do
				...
				self.DPanel_Job.Paint = function( self, w, h )
					...
					--draw.SimpleText(GAMEMODE.Config.currency .. ( v.pricesep or v.price ), "Amethyst.Font.JobSalary", 90, h / 2 + 4, DrawBoxTextColor or Color(255, 255, 255, 255))

					draw.RoundedBox(0, 0, 2, 75, h - 4, Color(0, 0, 0, 80))
					draw.RoundedBox(4, w - 75, h / 2 - 11, 55, 25, DrawSecondaryColor)

					draw.SimpleText(GAMEMODE.Config.currency .. ( v.pricesep or v.price ), "Amethyst.Font.CardItemMax", w - 48, h / 2 + 1, DrawBoxTextColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				end
	            ...
	            self.DModelPanel_ItemModel:SetFOV(v.amethyst_offsetfov or 30)
	            self.DModelPanel_ItemModel:SetCamPos(Vector(size, size, size))
	            self.DModelPanel_ItemModel:SetLookAt((mn + mx) * 0.5)
	            self.DModelPanel_ItemModel:SetPos( v.amethyst_offsetW or 7, v.amethyst_offsetH or 2 )
	            self.DModelPanel_ItemModel:SetSize( 60, 60 )
		        self.DModelPanel_ItemModel._Paint = self.DModelPanel_ItemModel.Paint

				local doHeightCalcTotal = math.ceil( table.Count( doCategoryVehicleList ) / 2 ) * doHeightCalc or 70
    ...
    self.DModelPanel_Item_Model:SetPos( 0, 0 )
	...
	self.DModelPanel_Item_Model.Think = function(self)
		...
		if Amethyst.Settings.CustomEntity[Amethyst.Value.entity] then
			self:SetFOV(Amethyst.Settings.CustomEntity[Amethyst.Value.entity].EntSetFOV or 80)
		end
	end
    ...
    self.DModelPanel_Item_Model.Paint = function(self, w, h)
        ...
        draw.SimpleText( GAMEMODE.Config.currency .. ( Amethyst.Value.pricesep or Amethyst.Value.price ), "Amethyst.Font.CardDescSubinfo", w / 2, 12, DrawSkinTextColor , 1, 1)
    end
    ...
    self.DLabel_Item_Desc.PerformLayout = function()
    	...
    	if Amethyst.Settings.CustomEntity[Amethyst.Value.name] then
			EntDescText = Amethyst.Settings.CustomEntity[Amethyst.Value.name].EntSetDesc or ""
		end
...
vgui.Register("Amethyst_Tab_Vehicles", PANEL, "DPanel")
