...
function PANEL:Init()
    ...
    self.DPanel_SectionTop.Paint = function(self, w, h)
		...
		draw.DrawText( string.upper(Amethyst.Language["food"] or "FOOD"), "Amethyst.Font.SectionTitle", 23, 5, Color(GetConVar("amethyst_m_tcolor_red"):GetInt(), GetConVar("amethyst_m_tcolor_green"):GetInt(), GetConVar("amethyst_m_tcolor_blue"):GetInt(), GetConVar("amethyst_m_tcolor_alpha"):GetInt()) or Color(255, 255, 255, 255), TEXT_ALIGN_LEFT )
    end
	...
	for _, doCatResult in pairs( doFetchCategories ) do
		...
		for k, v in pairs(FoodItems) do
			if not v.category then v.category = "Other" end
			if v.category == doCatResult.name then doHasCategory = true end
		end

		if doHasCategory then
			local doCategoryFoodList = {}

			for k, v in pairs(FoodItems) do
				if ( v.requiresCook and LocalPlayer():isCook() ) or ( not v.requiresCook ) then

					if v.customCheck != nil then
						if not v.customCheck then return end
					end
					if v.category != nil then
						if v.category == doCatResult.name then table.insert(doCategoryFoodList, v) end
					end
				end
			end

			local doCategoriesCalcHeight = 0
			for k, v in pairs( doCategoryFoodList ) do
				doCategoriesCalcHeight = math.ceil( table.Count( doCategoryFoodList ) / 2 ) * doHeightCalc + 40
			end
			...
			for k, v in pairs( doCategoryFoodList ) do
				...
				local doHeightCalcTotal = math.ceil( table.Count( doCategoryFoodList ) / 2 ) * doHeightCalc or 80
    ...
    self.DButton_Item_Action.PerformLayout = function()
        ...
        self.DButton_Item_Action.DoClick = function()
            RunConsoleCommand("darkrp", "buyfood", Amethyst.Value.name)
        end
    end
...
vgui.Register("Amethyst_Tab_Food", PANEL, "DPanel")
